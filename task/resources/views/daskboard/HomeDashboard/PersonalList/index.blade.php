        ...
        <div class="row g-4">
            <div class="col-sm-12 col-md-6 col-xl-12">
                <div class="h-100 bg-light rounded p-4">
                    ...
                    @foreach($personalList as $personalList)
                        <div class="d-flex align-items-center border-bottom py-3">
                            <form method="POST" action="{{ route('perosnalcomplete.update',$personalList->id)}}" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                            ...
                            <div class="w-100 ms-2">
                                <div class="d-flex w-80 align-items-center justify-content-between">
                                    <form method="POST" action="{{route('PersonalList.update',$personalList->id)}}">
                                        @csrf
                                        @method('PUT')
                                        <div type="submit" class="myText"> {{$personalList -> task }} </div>
                                        <script>
                            ...
                            <form action="{{route('PersonalList.destroy',$personalList->id)}}" method="POST" >
                                @csrf
                                @method('Delete')
                ...
                <div class="h-100 bg-light rounded p-4">
                    ...
                    @foreach($listcompleted as $listcompleted)
                        <div class="d-flex align-items-center border-bottom py-2 bg-teal-200">
                            <form method="POST" action="{{ route('personaluncomplete.update',$listcompleted->id)}}" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                            ...
                            <form action="{{route('PersonalList.destroy',$listcompleted->id)}}" method="POST" >
                                @csrf
                                @method('Delete')
                                <button type="submit" class="btn btn-sm"><i class="fa fa-times"></i></button>
                            </form>
